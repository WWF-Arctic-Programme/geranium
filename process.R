...
'interim' <- function(activity,group=NULL,aoi=NULL,season=list("max",NULL)[[1]]
   ...
   noneUse <- !allUse & (is.null(activity) | !(gr %in% names(industries)))
   anyUse <- !noneUse
   print(c(allUse=allUse,noneUse=noneUse,anyUse=anyUse))
   if (isShiny)
  ...
  #    blank1 <- blank
            ...
            if (is.na(s0)) {
               if (grepl("^max|maximum|maximal",season)) {
                  v <- aggregate(list(CF_name=v$CF_name,value=v$value)
                                ,by=list(CF_code=v$CF_code),max,na.rm=TRUE)
               }
            }
            else {
               patt <- "(^\\w+)\\s*-\\s*(\\w+$)"
   ...
   if (noneUse) {
      ...
      md <- read.csv(dir(path="requisite",pattern="scenario.*\\.csv$"
                    ...
                    ,check.names=FALSE)[,c("CF_code","CF_name")]
      md$value <- 9
      vulner <- list(dummy=md)
   }
   if (is.null(group)) {
  ...
  # sname <- if (anyUse) c('0','1','2') else c('9') 
   sname <- names(kwdLUT)
   if (isMap) {
  ...
  # cf <- v$CF_code %/% 1000
  # print(table(cf))
   cb <- chunk_band(amount,ifelse(isShiny,100,1000))
   if (F) {
      if (isStat) {
            ...
            for (s in sname) { ## '0' '1' '2'
               ...
               if (isStat) { ## if (!((!isPAC)&&(i!='zzz0'))) {
                  ...
                  b <- data.frame(code=names(am)
                                 ,name=v$CF_name[match(names(am),v$CF_code)]
                                 ,value1=b1#,value2=b2,value3=b3
                                # ,flag=if (noneUse) NA else kwdLUT[s]
                                 ,flag=kwdLUT[s]
                                 ,row.names=NULL)
                  ind <- grep("^value",colnames(b))[1]
                  b <- b[!is.na(b[[ind]]) & b[[ind]]>0.001,]
...
'crosstable' <- function(aoi=NULL) {
   ...
   res <- lapply(rules$activity |> sample(),function(industry) {
   ...
   colnames(res) <- gsub(pattRules,"\\2",colnames(res)) |> substr(1,300) #|> abbreviate(24)
   res[,1] <- sprintf("%.1f",as.numeric(res[,1]))
   res[is.na(res)] <- ""
   if (T) {
      md <- read.csv("requisite/Scenarios_Marxan_20200306e-abbrSeason.csv"
                    ,check.names=FALSE)
      ind <- match(rownames(res),md$CF_code)
      res <- cbind('CF name'=md$CF_name[ind],res)
   }
   res
}
'interimMap' <- function(industry="Mass tourism",group="\\d",season="max") {
   if ((group=="\\d")&&(season=="max")) {
...
if (F & !isShiny) {
   ...
   ursa:::.elapsedTime("interim -- begin")
   a2 <- interim(activity=list(noneActivity,"Tourism » Mass tourism","All human use")[[1]]
                ,aoi=aoi
                ,group="\\d",simplify="none")
   ursa:::.elapsedTime("interim -- end")
   print(a2$map)
   str(a2$stat)
