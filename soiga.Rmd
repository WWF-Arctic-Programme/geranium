...
output:
   flexdashboard::flex_dashboard:
      ...
      logoM: https://s2.googleusercontent.com/s2/favicons?domain_url=sev-in.ru
      logo: ./include/panda.svg
      favicon: ./include/panda.svg
      faviconM: https://s2.googleusercontent.com/s2/favicons?domain_url=sev-in.ru
...
output$uiMap <- renderUI({ ## editor-editModUI / selector-selectModUI / viewer-leafletOutput
  ...
  ## selector should be earlier editor
   ...
   if (F)
      NULL
   else if ((input$predefined %in% nameClick)&&((F & noAOI)||(input$economy %in% "none"))) { 
      cat("   selector:\n")
      ret <- selectModUI("selector")
   }
   else if (!input$region %in% nameEditor) { ## selected from list: "PAC "
      cat("   viewer:\n")
   ...
   ,conditionalPanel(condition="2==2" # "!!output.debug"
      ,fillRow(flex=NA
         ...
         ,conditionalPanel(condition="!output.pinEntered"
            ,passwordInput("pwd","User PIN",width="86px")
         )
         ,conditionalPanel(condition="!!output.pinEntered"
            ,fillCol(flex=NA
               ,renderText({
...
fillRow(flex=NA
   ,fillCol(flex=NA
      ...
      ,fillRow(flex=NA,height="50%"
         ...
         ,fillCol(flex=NA
           # ,br()
            ,conditionalPanel(condition="2*2==5" # "input.economy!='skip'"
...
fillCol(flex=c(NA)
   ,conditionalPanel(condition="input.scope=='action'"
      ...
      ,DT::renderDT({
         ...
         str(ctable)
         req(nrow(ctable)>0)
         if (all(ctable$'CAP' %in% c("n/a","N/A"))) {
            metrics <- rvRegionActivity()
