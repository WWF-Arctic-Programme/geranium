...
fillRow(flex=c(NA,NA,NA,NA,NA)
   ...
   ,selectInput("activity","Activity group",c(nameAllHuman,names(industries))
              # ,selected=ifelse(devel,sample(seasonList,1L),seasonList[1L])
               ,selected=c("Infrastructure","Mining","Shipping")
   ...
   ,{if (F)
      ...
      fillCol(flex=c(NA,1,1,1)
         ,DT::renderDT({
            aoi <- rvAOI()
            ctable <- rvActivityStat()
            cap <- indexCAP(aoi=aoi,ctable=ctable)
            cap0 <- indexCAP(aoi=NULL,ctable=ctable)
            da <- data.frame(Selection=round(cap,0),ArcNet=round(cap0,0))
