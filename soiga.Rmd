...
output:
   flexdashboard::flex_dashboard:
      ...
      favicon: https://i.stack.imgur.com/FzlQy.jpg
      orientation: columns
...
fillRow(flex=c(NA,1,NA)[1]
   ,shiny::selectInput("industry","Commercial activities"
...
fillRow(flex=c(NA,NA,NA),height="50%"
   ,conditionalPanel(condition="input.economy!='zzzskip'"
      ,shiny::selectInput("economy","Indices maps"
                     ,"---"
                     ,multiple=F
                     ,width="352px")
   )
   ...
   ,fixedRow(NULL
      ...
      ,column(3
         ,conditionalPanel(condition="input.actionHU=='0'"
            ,actionLink("actionHU","Show Commercial Activities (all)")
         )
      )
       ,column(1
   ...
   ,fillCol(flex=c(NA,1)
      ...
      ,DT::renderDT({
        ...
        # res <- result[grep('^NA',names(result))] |> as.data.frame()
         res <- result[c("NAOR","NACR","NAO","NAC")] |> as.data.frame()
         res <- rbind(res,data.frame(NAOR=result$meanNAOR,NACR=result$meanNACR,NAO=NA,NAC=NA))
         if (is.null(rvAOI())) {
         ...
         ,DT::renderDT({
           ...
           # str(ctable)
            ...
            ##~ cap <- indexCAPR(aoi=aoi
                           ...
                           ##~ ) |> band_mean()
            cap <- round(band_mean(indexCAPR(aoi=aoi,ctable=ctable)),0)
            hu <- round(band_mean(indexHumanUse(aoi=aoi,ctable=ctable)),2)
            res <- rbind(cap,hu)
            if (ncol(res)==1) {
            ...
            rownames(da) <- c("<abbr title='Conservation Action Priority index per cell'>CAPR</abbr>"
                             ,"<abbr title='Commercial Activities Amount per cell'>HUAR</abbr>")
