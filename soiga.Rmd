...
fillRow(flex=c(NA,1)
   ,shiny::selectInput("economy","Activity maps (only for visualization)"
                  ...
                  ,width="382px")
   ,HTML("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
  # ,actionLink("drawEconomy","Update map")
)
conditionalPanel(condition="input.industry!='none' || input.economy!='none'"
...
fillRow(flex=c(1,NA)
   ,fillCol(flex=c(NA,1)
      ...
      ,renderPlotly({
        ...
        # saveRDS(metrics,"C:/tmp/interim.rds")
         result <- metrics$'IND'
         localNAC <- concernNAC[,names(result)]/spatial_count(pu)
        ...
        # names(ct) <- sname
         ...
         mvalue <- meanNAC/length(IND$industry) # *metrics$nPU
         if (isTRUE(input$relative)) {
            IND$value <- 100*IND$value/metrics$'maxNAC'
            IND$ref <- 100*IND$ref/metrics$'maxNAC'
            mvalue <- 100*mvalue/metrics$'maxNAC'
         }
         p <- plot_ly(IND,x=~industry,y=~value,type="bar",orientation="v"
              ...
              add_trace(y=~ref,name='Domain',marker=list(color='rgb(204,204,204)')
                       ,visible="legendonly") %>%
              layout(NULL
                    ,xaxis=list(title=""),yaxis=list(title=c("IND","NACR")[2])
   ...
   ,fillCol(flex=c(NA,1)
     # ,HTML("Top NACR CF")
      ,HTML(paste0("<abbr title='Proportion of maximal NAC'>Top NACR CF</abbr>"))
      ,DT::renderDT({
...
### {data-header="Seasonal histogram" data-height=150}

```{r}
fillRow(flex=c(16,NA,24,NA,8)
   ,fillCol(flex=c(NA,1)
   ...
   ,fillCol(flex=c(NA,1)
      ...
      ,renderPlotly({
         ...
         p <- plot_ly(sc,x=~month,y=~value,type="bar",orientation="v") %>%
                  layout(NULL
                        ,xaxis=list(title=""),yaxis=list(title=c("SC IND","NACR")[2])
                       # ,margin=list(l=0,r=0,t=0)
                        )
         p
      })
