...
[Conservation Action Hotspots](#hotspots)

[How the assessment was made (TBD-to be developed)](#assessment)

[Dictionary (TBD-to be developed)](#dictionary)

[FAQ (TBD-to be developed)](#faq)
...
# Industry cut {#industry data-orientation="columns" .hidden}
...
output$listCF <- DT::renderDT({
  ...
  # b <- rvActivity()$stat
   ...
   if (hlink) {
     # b[[indCode]] <- paste0("<a href=#section-seasonIndustry>",b[[indCode]],"</a>")
      b[[indCode]] <- paste0("[",b[[indCode]],"](#seasonIndustry)")
   }
...
output$cfdata <- DT::renderDT({
   ...
   str(da)
   if (hlink) {
     # da$Industry <- paste0("[",da$Industry,"](#seasonIndustry)")
      da$Industry <- paste0("<a href=#section-seasonIndustry target=_top>",da$Industry,"</a>")
   }
  # b <- DT::datatable(b)
...
renderTable({
   ...
   if (T & hlink)
      md$CF_code <- paste0("[",md$CF_code,"](#seasonCF)")
...
```{r eval=isShiny}
renderText({
   cat("Cell comment (CF Seasonal):\n")
  # req(cf <- rvSelectCF())
   ind <- as.integer(input$cfdata_rows_selected)
   str(ind)
   req(length(ind)>0)
   industry <- unlist(industries)[ind]
   str(industry)
   activity <- gsub("\\d+","",names(industry))
   str(activity)
   cf <- input$cfcode
  # cf <- da$'CF Code'[ind]
...
### All compatibilities for a single Industry {#dummy data-width=500}
...
output$industrydata <- DT::renderDT({
   ...
   str(da)
   if (hlink)
      da$'CF Code' <- paste0("[",da$'CF Code',"](#seasonCF)")
  # b <- DT::datatable(b)
   b <- DT::datatable(da,rownames=FALSE,escape=FALSE,selection="single"
                       ,extensions="Scroller"
                       ,options=list(NULL
...
renderText({
  ...
  # req(cf <- rvSelectCF())
   req(industry <- rvSelectIndustry())
   req(ind <- as.integer(input$industrydata_rows_selected))
   activity <- names(industry)
