...
fillRow(flex=c(NA,NA,NA,NA,NA)
   ...
   ,selectInput("activity","Activity group",c(nameAllHuman,names(industries))
              # ,selected=ifelse(devel,sample(seasonList,1L),seasonList[1L])
               ,selected= if (staffOnly) c("Infrastructure","Mining","Shipping") else nameAllHuman
...
renderUI({
   ...
   b <- if (is.null(aoi)) list(b3,b2) else list(b1,b2)
...
fillRow(flex=c(NA,1,NA)
   ,shiny::selectInput("industry","Commercial activity acceptability (only for visualization)"
...
fillRow(flex=c(NA,1)
   ,shiny::selectInput("economy","Activity maps (only for visualization)"
                  ,"---"
                  ,multiple=T
                  ,width="382px")
   ,HTML("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
  # ,actionLink("drawEconomy","Update map after economy")
)
   ...
   ,fixedRow(NULL
      ...
      ,column(3
         ,conditionalPanel(condition="2>1"
            ,actionLink("showCommercial","Show Commercial Activities (all)")
         )
      )
       ,column(3
         ,conditionalPanel(condition="2>1"
            ,actionLink("showConservation","Show Conservation Action Priority (all)")
         )
      )
   )
   ,renderUI({
...
DT::renderDT({
   ...
   res <- result[grep("(nPU|nCF|ePA|^(area|pu))",names(result))] |> as.data.frame()
   cname <- colnames(res)
...
### {data-header="Conservation Concern Level by Activity" data-height=200}
...
fillCol(flex=c(NA,1)
   ...
   ,renderPlotly({
     ...
     # d <- concernSubset(concern,ctable=ctable)
      ...
      localNAC <- d[,names(result)]/ifelse(useNACR,spatial_count(pu),1)
      ref <- colSums(localNAC,na.rm=TRUE)
      if (!useNACR & !input$relative)
         ref <- ref*metrics$'nPU'/spatial_count(pu)
                             # spatial_count(pu)/metrics$'nPU'
      IND <- data.frame(industry=names(result),value=result,ref=ref)
     ...
     # names(ct) <- sname
      ...
      mvalue <- meanNAC/length(IND$industry)*ifelse(!useNACR & !input$relative,metrics$nPU,1)
      mvalue <- sum(colSums(localNAC,na.rm=TRUE))/length(IND$industry)
      if (!useNACR & !input$relative)
         mvalue <- mvalue*metrics$'nPU'/spatial_count(pu)
      if (isTRUE(input$relative)) {
      ...
      p <- plot_ly(IND,x=~industry,y=~value,type="bar",orientation="v"
           ...
           layout(NULL
                 ,xaxis=list(title="")
                 ,yaxis=list(title=ifelse(input$relative,"Indexes (%)"
                ...
                # ,showlegend=FALSE
                 ...
                 ,shapes=if (!input$relative) NULL else list(list(x0=~head(industry,1)
...
```{r}
fillRow(flex=c(8,NA,16,NA,24)
   ,fillCol(flex=c(NA,1)
      ,HTML("Indexes for the region")
      ,DT::renderDT({
         result <- rvRegionStats()
         req(result$nCF>0)
   ...
   ,{if (F)
      ...
      fillCol(flex=c(NA,1,1,1)
         ,DT::renderDT({
            ...
            cat("ctable:\n")
            str(ctable)
            shiny::showNotification(id="indexCAP",closeButton=FALSE,duration=120
                            ,paste("This action may take some time."
                                  ,"Please wait...")
                            ,type="warning")
            cap <- indexCAP(aoi=aoi,ctable=ctable) |> band_mean()
            shiny::removeNotification(id="indexCAP")
            print(cap)
            if (length(cap)==1)
               da <- data.frame(ArcNet=round(cap["domain"],0))
            else
               da <- data.frame(Selection=round(cap["aoi"],0),ArcNet=round(cap["domain"],0))
            rownames(da) <- "<abbr title='CAP index per cell'>CAPR</abbr>"
            DT::datatable(da,escape=FALSE
                         ,selection="none"
                         ,options=list(dom="t",ordering=F),class="compact"
                         )
         })
         ,HTML("Commercial Activities (PPU) = 21 (average for ArcNet = 4)")
         ,HTML("Conservation action priority (PPU) = 14 (average for ArcNet = 3")
         ,HTML("Conservation action priority (PPU = 194 (13451 sum for ArcNet))")
      )
   }
   ,HTML("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
   ,fillCol(flex=c(NA,1)
  ...
  # ,HTML("Top NACR CF")
   ...
   ,DT::renderDT({
     # result <- rvRegionStats() ## dummy returning; renew table
      ...
      colnames(ntable) <- paste0("<abbr title='Proportion of maximal NAC'>NAC</abbr>")
